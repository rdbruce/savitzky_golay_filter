...
sg = pf.SavGol(data_in = stock_data, num_elements = stock_data.size, window_size = 31, smoothing_degree = 1, return_degree =0, threaded = False)
smooth_data = sg.filter()
toc = time.perf_counter()
print(f"My function ran in {toc - tic:0.10f} seconds")

# print(smooth_data)

tic = time.perf_counter()
smooth_data2 = savgol_filter(stock_data, 31, 1, 0)
toc = time.perf_counter()
