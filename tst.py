...
for i in range(1, data_len):
    stock_data = np.append(stock_data, stock_data[i - 1] * (1 + 0.1/252 + 0.05 * math.sqrt(1/252) * np.random.default_rng().normal(0, 1)))
# print(stock_data.size)
print("Data Made.")
    
tic = time.perf_counter()
sg = pf.SavGol(data_in = stock_data, num_elements = stock_data.size, window_size = 7, smoothing_degree = 1, return_degree = 0, threaded = False)
smooth_data = sg.filter()
toc = time.perf_counter()
nonThread = toc - tic

tic = time.perf_counter()
sg = pf.SavGol(data_in = stock_data, num_elements = stock_data.size, window_size = 7, smoothing_degree = 1, return_degree = 0, threaded = True)
smooth_data = sg.filter()
toc = time.perf_counter()
thread = toc - tic
...
tic = time.perf_counter()
smooth_data2 = savgol_filter(stock_data, 7, 1, 0)
toc = time.perf_counter()
